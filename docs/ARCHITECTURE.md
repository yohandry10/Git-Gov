...
La fuente operativa es `gitgov/gitgov-server/src/server/routes.rs`. El router productivo contiene `158` registros Axum `.route(...)`, más Swagger UI en `/api-docs`; `/api-docs` sigue siendo un schema explorer parcial, no el contrato completo.
...
| `/agent-governance/context` | Bearer (admin o agent key con `agent_governance:read`) | Leer contexto read-only de rama, política, pipeline, deployment gates, riesgo y actividad sin autorizar ejecución ni crear evaluación |
...
`Platform Founder` no es el tenant interno GitGov. Es un principal de plataforma con `org_id=null`, fila activa en `platform_principals`, y `principal_type=platform_founder` en `/me`, usado para provisioning/bootstrap de tenants. El founder se autentica con API key GitGov (`Authorization: Bearer`), no con GitHub OAuth/device flow. GitHub puede identificar al operador Desktop para flujos de repo/tenant, pero no es el mecanismo de autenticación del superadmin. El tenant interno GitGov debe tratarse como un tenant normal de dogfooding: sus admins no crean tenants hermanos. La administración de tenants vive en `/platform/tenants`; `/orgs` conserva compatibilidad histórica pero no debe presentarse en UI como si un workspace estuviera creando otros workspaces.
