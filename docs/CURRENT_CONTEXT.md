...
- Current implementation branch: `product/KAN-122-change-risk-rule-trace`; KAN-122 is in progress and not yet merged/deployed.
...
- KAN-122 implementation status on this branch: implemented locally with Supabase migration/postcheck `v63`; backend routes `GET /change-risk/rules` and `GET /change-risk/evaluations/{evaluation_id}/trace`; `POST /change-risk/evaluations` now persists rule trace metadata; read access is Admin/Auditor while create remains Admin-only; Tauri DTO/client/commands and invoke registration are added; Control Plane store actions/state and `ChangeRiskPanel` trace UI are added; rule/trace helper code is split into `gitgov-server/src/handlers/change_risk_rules.rs`. Local validation so far: backend `cargo fmt`, `cargo check`, `cargo clippy -- -D warnings`, `cargo test --no-run`, focused real Postgres Change Risk tests (`2` passed), and real Postgres `v63` migration/postcheck in a rollback transaction; Tauri `cargo fmt`, `cargo check`, `cargo clippy -- -D warnings`, and tests (`49` passed); frontend focused ESLint, typecheck, focused store tests (`42` passed), full Vitest (`376` passed), lint, and build with the pre-existing Vite chunk warning; `git diff --check`; publication guard. Full backend `cargo test -- --test-threads=2` timed out twice locally after `10` and `15` minutes without useful failure output. PR, merge, production `v63`, Render deploy, and production smoke are still required.
- KAN-122 production migration note: after PR `#425` merged, the first production `v63` application added columns/indexes but postcheck failed because constraint idempotency checks were not scoped to `public.change_risk_evaluations`. Branch `fix/KAN-122-v63-constraint-scope` patches `supabase_schema_v63.sql` to scope each `pg_constraint` check by `conrelid`. The corrected `v63` was re-applied idempotently in production and `supabase_schema_v63_postcheck.sql` passed.
...
KAN-114 is in progress on `product/KAN-114-period-compliance-report-pdf-export`. KAN-113, KAN-112, KAN-111, KAN-110, KAN-109, and KAN-108 are complete and production-smoked.
...
- Do not add official regulatory wording, compliance scoring, certification claims, scheduler, or AI summaries without a new explicit product decision.
