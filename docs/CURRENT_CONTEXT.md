...
- Current implementation branch: `product/KAN-116-period-report-provenance-manifest`.
...
- KAN-116 implementation status on 2026-06-15: in progress on branch `product/KAN-116-period-report-provenance-manifest`. Implemented locally: Supabase migration/postcheck `v58`; `compliance_period_report_manifests`; `manifest_created`/`manifest_downloaded` custody actions; backend routes `POST /compliance/period-reports/{period_report_id}/provenance-manifests` and `GET /compliance/period-reports/{period_report_id}/provenance-manifests/{manifest_id}`; Tauri DTO/client/commands; Desktop store state/actions; `CompliancePeriodReportProvenancePanel`; focused Period Compliance Report integration assertions for first/second hash-chain manifests, PDF/source/access-log binding, unauthorized Auditor/tenant denial, no-claim flags, and unchanged source artifact hash; roadmap/architecture/report docs. Local validation passed so far: backend/Tauri `cargo fmt`, backend/Tauri `cargo check`, backend/Tauri `cargo clippy -- -D warnings`, Tauri tests (`49` passed), frontend `pnpm --dir gitgov typecheck`, frontend lint, focused store test (`35` passed), full frontend tests (`367` passed), frontend build, backend tests (`310` passed), focused backend test `period_compliance_report_aggregates_reviewed_reports_without_claims`, and local Docker Postgres migration chain `v55` through `v58` plus postchecks. Remaining before completion: `git diff --check`, publication guard, commit, push, PR checks, merge, Render deploy, production `v58` migration/postcheck, and production smoke.
