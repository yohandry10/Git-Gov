...
- KAN-98 implementation status on 2026-06-14: in progress on branch `feature/KAN-98-agent-read-context`. It adds `GET /agent-governance/context`, `agent_governance:read` scope support for agent keys, route-sensitive agent-key auth for read versus evaluate/dry-run, read-only context over existing branch/policy/pipeline/deployment-gate/risk/activity evidence, explicit response markers `read_only=true`, `will_authorize_execution=false`, and `mcp_surface=false`, and Postgres-backed integration tests for read success, invalid scope, manual-only denial, and read-only key isolation. Local validation passed before PR: focused `agent_governance_context` tests (`4` passed), `cargo check`, `cargo fmt --check`, `cargo clippy -- -D warnings`, and full backend tests (`296` passed). PR/check/merge/production validation remain pending.
