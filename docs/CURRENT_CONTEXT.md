...
- Current implementation branch: `product/KAN-120-first-governed-repo-wizard`; KAN-120 is implemented locally and pending PR, merge, Render deploy, and production smoke.
- Current implementation ticket: GitHub issue `#418`, `KAN-120 First Governed Repo Setup Integration Wizard MVP`.
...
- KAN-120 implementation status on 2026-06-16: local implementation adds backend routes `GET /onboarding/first-governed-repo/state`, `POST /onboarding/first-governed-repo/runs`, `PATCH /onboarding/first-governed-repo/runs/{run_id}`, `POST /onboarding/first-governed-repo/runs/{run_id}/validate`, `POST /onboarding/first-governed-repo/runs/{run_id}/plan`, and `POST /onboarding/first-governed-repo/runs/{run_id}/complete`; backend wizard DTOs and handler split into setup, helpers, and wizard endpoints; Tauri DTO/client/commands and invoke registration; Control Plane store state/actions; and `FirstGovernedRepoSetupPanel` wizard controls. No DB migration is required because KAN-120 uses the KAN-80 table/baseline. Local validation so far: backend fmt/check/clippy, focused real Postgres `first_governed_repo_setup` test with local `TEST_DATABASE_URL=127.0.0.1:5433`, full backend Postgres suite with `--test-threads=2` (`311` passed), Tauri fmt/check/clippy/tests (`49` passed), frontend typecheck/lint/focused store/full tests (`373` passed)/build, `git diff --check`, and publication guard. Remaining: PR checks, merge, Render deploy, and production smoke.
