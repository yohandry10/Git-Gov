...
- KAN-114 implementation status on 2026-06-15: local implementation complete and validated on branch `product/KAN-114-period-compliance-report-pdf-export`. Local code adds Supabase migration/postcheck `v56` for `compliance_period_report_pdf_exports`; backend routes `POST/GET /compliance/period-reports/{period_report_id}/pdf-export` and `GET /compliance/period-reports/{period_report_id}/pdf-export/download`; Tauri models/client/commands; Desktop period report PDF controls; store state/actions/tests; architecture/roadmap/report docs. The backend period PDF handlers were split into `gitgov-server/src/handlers/compliance_period_reports/pdf_exports.rs`, and Desktop store actions were split into focused Period Report and Framework Review Report artifact action modules to keep maintained files below the normal size target. Validation passed after the refactor: backend `cargo check`, `cargo clippy -- -D warnings`, focused real Postgres integration test `period_compliance_report_aggregates_reviewed_reports_without_claims`, full backend real Postgres suite (`310` passed), local `v56` migration/postcheck through ignored `DATABASE_URL`, Tauri `cargo check`, `cargo clippy -- -D warnings`, Tauri tests (`49` passed), frontend `pnpm --dir gitgov typecheck`, `pnpm --dir gitgov lint`, focused store test (`35` passed), full frontend tests (`367` passed), frontend build, `git diff --check`, and publication guard. The focused integration test covers the full KAN-113 evidence chain plus KAN-114 PDF creation/download, assignment/tenant isolation, PDF bytes/hash, no secret fixture leakage, unchanged source period report hash, audit log, no-claim flags, and no Agent Governance evaluation creation. Remaining before complete: commit/push/PR/checks/merge, Render deploy, production v56 migration/postcheck, production smoke, and final context update.
