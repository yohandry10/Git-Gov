...
- KAN-82 implementation status: local branch `feature/KAN-82-platform-principals` adds `platform_principals` through `supabase_schema_v34.sql`, backend auth now resolves Platform Founder from an active/break-glass `platform_principals` row, `/me` includes `platform_principal_id`, Desktop/Tauri types accept the new field, and the old `VITE_FOUNDER_GITHUB_LOGIN`/`VITE_FOUNDER_LOGIN` founder-gating path was removed. Product decision: superadmin/founder authenticates by GitGov API key plus platform principal row, not by GitHub OAuth/device flow; GitHub remains operator/repo identity for tenant workflows.
- KAN-82 production DB validation on 2026-06-13: `supabase_schema_v34.sql` was applied manually through ignored local `DATABASE_URL`; `v34_postcheck.sql` returned `PASS` for `platform_principals.table`, `platform_principals.constraints`, and `platform_principals.bootstrap_founder`. The production DB now has active `bootstrap-admin` platform founder metadata without printing or committing secrets. Code deployment/PR merge for KAN-82 is still pending.
