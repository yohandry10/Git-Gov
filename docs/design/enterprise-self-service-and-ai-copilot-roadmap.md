...
- A full wizard shell that can persist operator skip/retry decisions across sessions.
- Secret-safe OAuth/API-key connection status per provider.
- Recommended workflow/policy preset generated from selected tools.
- Guided cross-surface routing after operators complete setup steps, including persisted skip/retry
  decisions and clearer return paths back into the wizard.
