...
- `KAN-80` adds the first governed repo setup slice: Admin-selected repo/branch, provider/module
  ...
  integration wizard.
- `KAN-120` turns the existing KAN-80 setup into a manual-first integration wizard. Admins can
  load state, create/resume the run, validate provider evidence already visible to GitGov, generate a
  baseline plan, and complete the first result using `/onboarding/first-governed-repo/*`. Auditors can
...
- `KAN-107`: Framework Review Report manual review workflow. Admins can mark an existing report as
...
- `KAN-119`: Period Compliance Report Share Packages. Admins can create a manual/offline JSON bundle
  for an already `reviewed` Period Compliance Report only when the source report, PDF export, and
  provenance manifest already exist. The package binds the period JSON hash, PDF hash, manifest hash,
  review snapshot, retention snapshot, no-claim flags, and safe verification instructions. Admins and
  authorized Auditors can list/download packages; Admins can revoke future downloads. It is manual
  sharing only: no public links, email delivery, scheduler, DOCX/formal template, certification,
  legal attestation, compliance score, official regulatory claim, AI summary, BYOM/MCP/chatbot, or
...
- `KAN-147` adds a compact provider setup guidance layer in Enterprise Adoption. It translates
  existing provider health into manual `Connect`, `Retry`, `Review`, and `Skipped` actions per
  provider without OAuth, provider API calls, secret reads, provider/repository mutation, or backend
  changes.
- `KAN-148` adds navigation-only links from those setup actions to the existing surfaces that own
  each manual workflow: Settings/System, Governance Evidence, Action Center, and Enterprise
  Adoption profile. It keeps provider setup non-blocking and does not execute provider setup.
- `KAN-149` persists human setup decisions in the existing Enterprise Adoption profile JSON:
  `retry-later`, `reviewed`, and `intentionally-skipped`. It reuses the existing profile save path
  and does not add provider automation, OAuth, provider API calls, new routes, or a new DB table.

Future scope:

- A full wizard shell with richer sequencing, resume/return state, and clearer customer handoff.
- Secret-safe OAuth/API-key connection status per provider.
- Recommended workflow/policy preset generated from selected tools.
- Guided cross-surface routing after operators complete setup steps, including clearer return paths
...
- `KAN-122` adds rule-level explainability to that advisory: a deterministic
  `change_risk_rules.v1` catalog, persisted triggered/non-triggered rules, evaluation trace,
  trace hash, catalog/trace APIs, Auditor read access, and a Desktop `Why this risk?` view. It keeps
  creation Admin-only and does not add enforcement, scoring, provider/repo mutation, AI, or claims.
- `KAN-123` adds manual review and mitigation notes over an explained Change Risk evaluation:
  `review_status`, reviewer, timestamp, safe review/mitigation/decision notes, read/update review
  APIs, audit event `change_risk_review_updated`, and Desktop `Manual Review`. Admins update;
  Auditors read. It does not block releases, approve deployments, mutate gates/providers/repos, use
  AI, depend on Agent Governance, or create compliance/certification/legal claims.
- `KAN-124` adds a manual Change Risk review queue by filtering existing evaluations by
  `review_status`, exposing the same filter in Desktop, and keeping review queue behavior
...
- `KAN-129` starts the productized executive view with read-only
  ...
  claim.
- `KAN-130` adds the first filter layer to that executive view: repository search, environment,
  posture, gate decision, risk level, and review status. It reuses the same read-only route and
  keeps filters as triage only, not enforcement or compliance scoring.
- `KAN-131` adds hashable JSON snapshot export for the filtered executive view. It persists
  `executive_governance_snapshots`, download/archive metadata, and no-claim flags. It is a
  verifiable review artifact, not a compliance report, score, release decision, deployment action,
  AI output, or Agent Governance workflow.
...
- Future filters by team, repo criticality, ownership metadata, and richer violation taxonomy.
...
- Organization/repository report templates beyond the KAN-118/KAN-119 manual report/profile/share
  package MVPs.
...
- `KAN-132` starts the block with Desktop Workspace native terminal session history. It captures
  ...
  claims.
- `KAN-133` adds safe repo/branch context to that native terminal header. It resolves local Git
  context through Tauri and `git2::Repository::discover`, shows `repo:branch`, detached, pending, or
...
- `KAN-141` lets the compact branch gate badge open the existing native terminal Governance Context
  drawer. This gives developers one-click drill-down into the same read-only Deployment Gate, Change
  Risk, and Executive Governance context from KAN-135 without adding a new modal, backend endpoint,
  evidence write, command interception, enforcement model, provider/repo mutation, or compliance
  claim.
- `KAN-143` adds a local provider/tool quick-command safety registry to the existing native terminal
  quick-command menu. The MVP adds exact-allowlisted, insert-only commands for Terraform local
  validation, Kubernetes local config, Docker Compose config, and Helm lint. Commands with network
  access, secret exposure, mutation, shell chaining, redirection, or cloud/provider API calls remain
  rejected or out of scope. It also replaces `git remote -v` with `git remote` so remote URLs are
  not printed. It does not auto-run commands, call the backend, mutate providers/repos, create
  enforcement evidence, or make compliance/certification claims.
- `KAN-144` adds local provider/tool context detection for the native terminal quick-command menu.
  The MVP detects Terraform, Docker Compose, Helm, and Kubernetes context from bounded safe
  file/directory names only, stores context by native terminal session, and groups detected safe
  commands under `Available in this workspace`. It does not read file contents, return absolute
  paths, auto-run commands, call the backend, mutate providers/repos/clusters, create enforcement
  evidence, or make compliance/certification claims.
- `KAN-145` adds quiet disabled action previews to the same native terminal quick-command menu when
  local provider/tool context is detected. The previews are advisory text only and explain categories
  intentionally absent from shortcuts, such as state-changing tool actions, provider API actions,
  secret/value inspection, and repository writes. They do not contain runnable command strings, do
  not render as buttons, do not insert terminal input, do not intercept/block commands, and do not
  mutate providers, repositories, clusters, deployments, or compliance evidence.
- `KAN-146` makes the native terminal non-interception contract explicit and tested. Manual terminal
  input is still observed locally for session history and safe context refresh, but the input bytes
  are forwarded unchanged to the native PTY by default. The contract records `interception=none`,
  `policyEvaluation=not-run`, and `mutatesInput=false`; it does not add policy enforcement,
  command approval, command blocking, backend calls, provider/repo mutation, or compliance evidence.
...
- Onboarding flow:
  ...
  - connect Jenkins or SonarQube when used.
    - KAN-147 now translates provider health into `Connect`, `Retry`, `Review`, and `Skipped`
      guidance per provider inside Enterprise Adoption.
    - KAN-148 links those guidance actions to Settings/System, Governance Evidence, Action Center,
      and the Enterprise Adoption profile without starting OAuth or provider mutation.
    - KAN-149 persists human setup decisions inside the existing adoption profile JSON so operators
      can remember retry-later, reviewed, and intentionally skipped providers across sessions.
    - Actual OAuth/API-key connection automation and provider installation remain future optional
      packaging.
