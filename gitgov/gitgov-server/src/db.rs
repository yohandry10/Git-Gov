...
fn enterprise_release_approval_from_row(row: &PgRow) -> EnterpriseReleaseApprovalRecord {
    EnterpriseReleaseApprovalRecord {
        ...
        risk_acceptance_reason: row.get("risk_acceptance_reason"),
...
fn deployment_gate_authorization_from_row(row: &PgRow) -> DeploymentGateAuthorizationRecord {
    ...
    DeploymentGateAuthorizationRecord {
        ...
        break_glass_used: row.get("break_glass_used"),
...
fn change_risk_evaluation_from_row(row: &PgRow) -> ChangeRiskEvaluationRecord {
    ChangeRiskEvaluationRecord {
        ...
        recommended_manual_actions: serde_json::from_value(row.get("recommended_manual_actions"))
            .unwrap_or_default(),
        triggered_rules: serde_json::from_value(row.get("triggered_rules")).unwrap_or_default(),
        non_triggered_rules: serde_json::from_value(row.get("non_triggered_rules"))
            .unwrap_or_default(),
...
mod exports;
mod feature_cli_policy_drift;
mod governance_events;
mod jira_coverage;
mod jira_ingest;
mod jobs;
mod noncompliance_detection;
mod noncompliance_records;
mod org_invitations;
mod org_users;
mod orgs;
mod pipeline;
...
// -----------------------------------------------------------------------------
// Original db.rs migration archive (commented on purpose).
// The live implementation above is split into db/*.rs modules. Keep this
...
// Some destinations have continuation markers because the original db.rs kept
// related methods in separated ranges.
// -----------------------------------------------------------------------------

//
// ===== PART 0 -> live db.rs root definitions/imports/types/helpers =====
// use crate::models::*;
// use sha2::Digest;
// use sqlx::postgres::{PgPoolOptions, PgRow};
// use sqlx::{PgPool, QueryBuilder, Row};
...
//
//
// ===== PART 3 -> src/db/enterprise.rs (enterprise) =====
...
//
//
// ===== PART 2 (continuation) -> src/db/orgs.rs (orgs) =====
...
//
//
// ===== PART 4 -> src/db/events_github.rs (events_github) =====
...
//
//
// ===== PART 5 -> src/db/events_client.rs (events_client) =====
...
//
//
// ===== PART 6 -> src/db/exports.rs (exports) =====
...
//
//
// ===== PART 7 -> src/db/pipeline.rs (pipeline) =====
...
//
//
// ===== PART 8 -> src/db/jira_ingest.rs (jira_ingest) =====
...
//
//
// ===== PART 9 -> src/db/jira_coverage.rs (jira_coverage) =====
...
//
//
// ===== PART 10 -> src/db/evidence_packets.rs (evidence_packets) =====
...
//
//
// ===== PART 7 (continuation) -> src/db/pipeline.rs (pipeline) =====
...
//
//
// ===== PART 11 -> src/db/policy.rs (policy) =====
...
//
//
// ===== PART 12 -> src/db/auth_api_keys.rs (auth_api_keys) =====
...
//
//
// ===== PART 13 -> src/db/pr_admin.rs (pr_admin) =====
...
//
//
// ===== PART 14 -> src/db/noncompliance_records.rs (noncompliance_records) =====
...
//
//
// ===== PART 15 -> src/db/noncompliance_detection.rs (noncompliance_detection) =====
...
//
//
// ===== PART 16 -> src/db/compliance.rs (compliance) =====
...
//
//
// ===== PART 11 (continuation) -> src/db/policy.rs (policy) =====
...
//
//
// ===== PART 6 (continuation) -> src/db/exports.rs (exports) =====
...
//
//
// ===== PART 17 -> src/db/governance_events.rs (governance_events) =====
...
//
//
// ===== PART 19 -> src/db/violations_privacy_sessions.rs (violations_privacy_sessions) =====
//     // ========================================================================
...
//
//
// ===== PART 20 -> src/db/org_users.rs (org_users) =====
...
//
//
// ===== PART 21 -> src/db/org_invitations.rs (org_invitations) =====
...
//
// ===== PART 22 -> src/db/chat_queries_core.rs (chat_queries_core) =====
...
//
// ===== PART 23 -> src/db/chat_queries_quality.rs (chat_queries_quality) =====
...
//
// ===== PART 24 -> src/db/chat_queries_release.rs (chat_queries_release) =====
...
//
// ===== PART 25 -> src/db/feature_cli_policy_drift.rs (feature_cli_policy_drift) =====
...
//
//
// ===== PART 26 -> live db.rs root structs: Job, JobMetrics, ViolationDecision =====
// #[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
// pub struct Job {
//     pub id: String,
//     pub org_id: String,
...
//
//
// ===== PART 27 -> src/db/tests.rs (db module tests) =====
// #[cfg(test)]
// mod tests {
//     use super::*;
