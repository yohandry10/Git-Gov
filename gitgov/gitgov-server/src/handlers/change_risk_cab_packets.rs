...
const CHANGE_RISK_CAB_REVIEW_PENDING: &str = "pending_review";
const CHANGE_RISK_CAB_REVIEW_REVIEWED: &str = "reviewed";
const CHANGE_RISK_CAB_REVIEW_ACCEPTED_RISK: &str = "accepted_risk";
const CHANGE_RISK_CAB_REVIEW_NEEDS_MITIGATION: &str = "needs_mitigation";
const CHANGE_RISK_CAB_REVIEW_RETURNED_TO_OWNER: &str = "returned_to_owner";
const CHANGE_RISK_CAB_REVIEW_REJECTED: &str = "rejected";
...
fn normalize_change_risk_cab_packet_review_request(
    payload: &mut ChangeRiskCabPacketReviewRequest,
) -> Result<(), Vec<String>> {
    let mut errors = Vec::new();
    normalize_release_approval_optional_text(&mut payload.org_name);
    payload.review_status = payload.review_status.trim().to_ascii_lowercase();
    if ![
        CHANGE_RISK_CAB_REVIEW_PENDING,
        CHANGE_RISK_CAB_REVIEW_REVIEWED,
        CHANGE_RISK_CAB_REVIEW_ACCEPTED_RISK,
        CHANGE_RISK_CAB_REVIEW_NEEDS_MITIGATION,
        CHANGE_RISK_CAB_REVIEW_RETURNED_TO_OWNER,
        CHANGE_RISK_CAB_REVIEW_REJECTED,
    ]
    .contains(&payload.review_status.as_str())
    {
        errors.push(
            "review_status must be pending_review, reviewed, accepted_risk, needs_mitigation, returned_to_owner, or rejected."
                .to_string(),
        );
    }
    if let Err(error) =
    ...
    if let Err(error) =
        normalize_safe_change_risk_review_text(&mut payload.follow_up_owner, "follow_up_owner")
    {
        errors.push(error);
    }
    if matches!(
    ...
    {
        errors.push(
            "decision_reason is required when review_status is accepted_risk, returned_to_owner, or rejected."
                .to_string(),
        );
    }
    if payload.review_status == CHANGE_RISK_CAB_REVIEW_NEEDS_MITIGATION
        && (payload.mitigation_notes.is_none() || !payload.follow_up_required)
    {
        errors.push(
            "mitigation_notes and follow_up_required=true are required when review_status is needs_mitigation."
                .to_string(),
        );
    }
    if payload.review_status != CHANGE_RISK_CAB_REVIEW_NEEDS_MITIGATION {
        payload.follow_up_required = false;
        payload.follow_up_owner = None;
    }
...
fn change_risk_cab_packet_review_response(
    packet: ChangeRiskCabPacketRecord,
...
pub async fn get_change_risk_cab_packet_review(
    Extension(auth_user): Extension<AuthUser>,
    State(state): State<Arc<AppState>>,
    Path(packet_id): Path<String>,
    Query(mut query): Query<ChangeRiskCabPacketQuery>,
    ...
    match state.db.get_change_risk_cab_packet(&org_id, &packet_id).await {
        Ok(Some(packet)) => {
            let audit = AdminAuditLogEntry {
                id: Uuid::new_v4().to_string(),
                actor_client_id: auth_user.client_id.clone(),
                action: "change_risk_cab_packet_review_viewed".to_string(),
                target_type: Some("change_risk_cab_packet".to_string()),
                target_id: Some(packet.packet_id.clone()),
                metadata: json!({
                    "org_id": org_id,
            ...
            if let Err(e) = state.db.insert_admin_audit_log(&audit).await {
                tracing::warn!(error = %e, "Failed to write admin audit log (change risk CAB packet review view)");
            }
            (
                StatusCode::OK,
                Json(change_risk_cab_packet_review_response(packet)),
            )
                .into_response()
        }
        Ok(None) => (
            ...
            .into_response(),
        Err(e) => {
            tracing::error!(error = %e, org_id = %org_id, packet_id = %packet_id, "Failed to get change risk CAB packet review");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Internal database error" })),
            )
...
pub async fn update_change_risk_cab_packet_review(
    ...
    State(state): State<Arc<AppState>>,
    Path(packet_id): Path<String>,
    Json(mut payload): Json<ChangeRiskCabPacketReviewRequest>,
) -> impl IntoResponse {
    if let Err(resp) = require_admin(&auth_user) {
        return resp.into_response();
    }
    ...
    if let Err(errors) = normalize_change_risk_cab_packet_review_request(&mut payload) {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({ "error": "Invalid Change Risk CAB packet review request", "details": errors })),
        )
            .into_response();
    }
    let org_id = match resolve_change_risk_org(&state, &auth_user, payload.org_name.as_deref()).await
    ...
    let current = match state.db.get_change_risk_cab_packet(&org_id, &packet_id).await {
        ...
        Err(e) => {
            tracing::error!(error = %e, org_id = %org_id, packet_id = %packet_id, "Failed to authorize CAB packet review update");
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Internal database error" })),
            )
    ...
    match state
        .db
        .update_change_risk_cab_packet_review(&UpdateChangeRiskCabPacketReviewInput {
            org_id: &org_id,
            packet_id: &packet_id,
            review_status: &payload.review_status,
            ...
            follow_up_owner_safe: payload.follow_up_owner.as_deref(),
        })
        .await
    {
        Ok(Some(packet)) => {
            let audit = AdminAuditLogEntry {
                id: Uuid::new_v4().to_string(),
                actor_client_id: auth_user.client_id.clone(),
                action: "change_risk_cab_packet_review_updated".to_string(),
                target_type: Some("change_risk_cab_packet".to_string()),
                target_id: Some(packet.packet_id.clone()),
                metadata: json!({
                    "org_id": org_id,
                    ...
                    "has_mitigation_notes_safe": packet.mitigation_notes_safe.is_some(),
                    "has_decision_reason_safe": packet.decision_reason_safe.is_some(),
                    "follow_up_required": packet.follow_up_required,
                    "has_follow_up_owner_safe": packet.follow_up_owner_safe.is_some(),
            ...
            if let Err(e) = state.db.insert_admin_audit_log(&audit).await {
                tracing::warn!(error = %e, "Failed to write admin audit log (change risk CAB packet review update)");
            }
            (
                StatusCode::OK,
                Json(change_risk_cab_packet_review_response(packet)),
        ...
        Ok(None) => (
            ...
            .into_response(),
        Err(e) => {
            tracing::error!(error = %e, org_id = %org_id, packet_id = %packet_id, "Failed to update change risk CAB packet review");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Internal database error" })),
            )
