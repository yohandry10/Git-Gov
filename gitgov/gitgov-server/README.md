...
| GET | `/api-docs` | Swagger UI for partial schema exploration |
| GET | `/api-docs/openapi.json` | Partial OpenAPI schema generated by `openapi::build_openapi_spec()` |

### Webhooks & Events

| Method | Path | Description |
|--------|------|-------------|
| POST | `/webhooks/github` | GitHub webhook receiver for push, create, PR, review/comment, check, suite, and status evidence |
| POST | `/webhooks/jira` | Signed Jira webhook receiver (HMAC) |
| POST | `/events` | Client events (batch with idempotency) |
| POST | `/audit-stream/github` | GitHub audit-log batch ingest |
...
| GET | `/governance-events` | GitHub governance/audit events |
| GET | `/pr-merges` | PR merge evidence |
...
| POST | `/signals/{signal_id}` | Update signal status (investigate/dismiss) |
| POST | `/signals/{signal_id}/confirm` | Confirm signal |
| POST | `/signals/detect/{org_name}` | Trigger bypass detection |
| GET/POST | `/violations/{violation_id}/decisions` | List/add violation decisions |

### Policy

| Method | Path | Description |
|--------|------|-------------|
| GET | `/policy/{repo_name}` | Get policy for repo |
| POST | `/policy/check` | Evaluate policy for branch/PR context |
| GET | `/policy/{repo_name}/history` | Policy change history |
| PUT | `/policy/{repo_name}/override` | Override repo policy |
| GET/POST | `/policy/{repo_name}/requests` | List/create policy change requests |
| POST | `/policy/requests/{request_id}/approve` | Approve policy change request |
| POST | `/policy/requests/{request_id}/reject` | Reject policy change request |
| GET/POST | `/policy/drift-events` | List/ingest policy drift evidence |
...
| GET | `/integrations/jira/tickets/{ticket_id}` | Jira ticket detail |
...
| POST | `/jobs/{job_id}/retry` | Retry dead job |
