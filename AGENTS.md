...
- Product vulnerability trend enforcement ticket `KAN-28` adds `.github/workflows/product-vulnerability-review-trend-enforcement.yml` and enforcement switches to `scripts/control-plane/generate_product_vulnerability_review_trend_report.ps1`. It fails when latest failures exceed `0`, latest findings exceed the accepted baseline of `1`, findings/failures increase, or the latest successful review run lacks a parseable artifact. Local enforcement validation against run `25157972836` passed with `5` pass, `1` expected finding, and `0` fail.
