...
- Release Governance Profile Policy ticket `KAN-45` is active on branch `product/KAN-45-release-governance-profile-policy`. It turns the KAN-44 decision into explicit adoption-profile configuration: default `record-only` / `disabled` release governance, optional `advisory`, `approval-required`, and `quorum-required` modes, dashboard controls, backend profile validation, adoption pack output, workflow template manifest output, and docs `docs/design/release-governance-profile-policy-mvp.md` plus `docs/reports/release-governance-profile-policy-2026-05-01.md`. The default remains non-blocking; non-`record-only` modes require the `formal-approval` module. No provider tokens, `.env` values, Authorization headers, or customer secret values are read or written.
