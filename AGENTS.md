...
- Product vulnerability automation ticket `KAN-25` operationalizes the KAN-24 runner through `.github/workflows/product-vulnerability-review.yml`. The workflow runs weekly in `DependenciesOnly` mode and manually supports `DependenciesOnly`, `StaticOnly`, `RuntimeSmoke`, and `Full`; artifacts are uploaded as `product-vulnerability-review-{run_id}`. Runbook: `docs/runbooks/product-vulnerability-review-automation.md`. Report: `docs/reports/product-vulnerability-review-automation-2026-04-30.md`. The runner was made cross-platform for GitHub-hosted Ubuntu while preserving local Windows use.
