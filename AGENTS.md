...
- Dashboard Workflow Template Pack ticket `KAN-34` continues Enterprise Self-Service Adoption by exposing workflow template pack download inside the Enterprise Adoption dashboard. Active branch `product/KAN-34-dashboard-workflow-template-pack` adds `buildEnterpriseWorkflowTemplatePack`, `buildEnterpriseWorkflowTemplatePackFilename`, a `Workflows` download action in `EnterpriseAdoptionPanel.tsx`, focused helper tests, `docs/design/dashboard-workflow-template-pack-mvp.md`, and `docs/reports/dashboard-workflow-template-pack-2026-04-30.md`. The dashboard pack is a single JSON file with manifest, README text, and workflow file contents. It does not read `.env`, does not read/print secret values, and does not mutate customer repositories. Vercel AI SDK remains pending until Enterprise Self-Service Onboarding is more complete.
