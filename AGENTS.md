...
- Formal Release Approval ticket `KAN-37` continues Enterprise Self-Service Adoption by adding the first append-only release approval API. Active branch: `product/KAN-37-formal-release-approval`. It adds `GET /enterprise/release-approvals`, `POST /enterprise/release-approvals`, `enterprise_release_approvals`, Supabase migration `v24`, evidence packet hash binding, risk acceptance expiration, admin-only org scope, stale-auth-cache blocking for this sensitive admin route, and audit logging. Design: `docs/design/formal-release-approval-mvp.md`. Report: `docs/reports/formal-release-approval-2026-04-30.md`. Local backend validation passed with `cargo test`, `cargo check`, `cargo clippy -- -D warnings`, `git diff --check`, and `.\scripts\security\publication_guard.ps1`. Vercel AI SDK Copilot remains pending until KAN-37 is merged and validated.
