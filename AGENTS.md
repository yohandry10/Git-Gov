...
- Provider Connection Validation ticket `KAN-36` continues Enterprise Self-Service Adoption by adding secret-safe direct provider credential/reachability checks for onboarding. Branch `product/KAN-36-provider-connection-validation` adds `scripts/control-plane/validate_enterprise_provider_connections.ps1`, `docs/design/provider-connection-validation-mvp.md`, and `docs/reports/provider-connection-validation-2026-04-30.md`. The validator supports GitHub, Jira, Jenkins, SonarQube, Render, and Vercel; reads credentials only from ignored env files or process environment; reports required config names and sanitized statuses; and does not print secret values, mutate provider state, create webhooks, create GitHub Actions variables/secrets, or mutate customer repositories. Formal enterprise release approval and Vercel AI SDK Copilot remain pending.
