...
- Workflow Template Generation ticket `KAN-33` continues Enterprise Self-Service Adoption by converting the adoption profile into reviewed GitHub Actions workflow template packs. Active branch `product/KAN-33-workflow-template-generation` adds `scripts/control-plane/generate_enterprise_workflow_templates.ps1`, `docs/design/workflow-template-generation-mvp.md`, and `docs/reports/workflow-template-generation-2026-04-30.md`. The generator writes ignored output under `out/enterprise-workflow-templates/` with `README.md`, `workflow-template-manifest.json`, and selected `.github/workflows/*.yml`; the ExampleCo profile generates `13` templates. It does not read `.env`, provider tokens, or secret values, and it does not mutate customer repositories automatically. Workflow installation remains a reviewed/manual or future explicitly authorized step.
