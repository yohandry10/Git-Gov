...
- Adoption Profile Dashboard ticket `KAN-30` continues Enterprise Self-Service Adoption by adding the first admin UI for the KAN-29 profile shape. Branch `product/KAN-30-adoption-profile-dashboard` adds `gitgov/src/components/control_plane/EnterpriseAdoptionPanel.tsx`, typed adoption pack helpers in `dashboard-helpers.ts`, validation tests, `docs/design/adoption-profile-dashboard-mvp.md`, and `docs/reports/adoption-profile-dashboard-mvp-2026-04-30.md`. The dashboard JSON export remains secret-safe: it contains variable/secret names only and does not read local env files or provider tokens.
