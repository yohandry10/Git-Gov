...
- Release Governance Evaluator ticket `KAN-46` is the active implementation branch `product/KAN-46-release-governance-evaluator` and PR `#146`. It adds admin endpoint `GET /enterprise/release-governance/evaluate`, Tauri command `cmd_server_evaluate_enterprise_release_governance`, dashboard evaluation UI in `ReleaseApprovalPanel`, approver role metadata through `evidence_summary.approver_role`, and docs `docs/design/release-governance-evaluator-mvp.md` plus `docs/reports/release-governance-evaluator-2026-05-01.md`. The evaluator reads the KAN-45 profile policy and KAN-37 approval records, returns `recorded`, `advisory-warning`, `approved`, `would-block`, or `blocked`, and keeps `record-only` non-blocking by default. No provider tokens, `.env` values, Authorization headers, or customer secret values are read or written.
