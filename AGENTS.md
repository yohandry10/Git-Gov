...
- Evidence packets ticket `KAN-23` is the selected first product enhancement after the GPT 5.5 deep research report: implement ticket-scoped evidence packets before a Vercel AI SDK copilot. MVP route is `GET /evidence/packets/tickets/{ticket_id}` with JSON download/hash UI; Vercel AI SDK, MCP, PR/release packet types, and persisted packet export history remain follow-ups.
