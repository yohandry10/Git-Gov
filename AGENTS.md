...
- Google AI SDK Copilot Provider ticket `KAN-41` switches the practical production AI activation path from Vercel AI Gateway to direct Google Gemini through `@ai-sdk/google`, because Vercel AI Gateway generation required a billing card and the project already has a Gemini integration. Branch: `product/KAN-41-google-ai-sdk-copilot`. Vercel production env now has `GOOGLE_GENERATIVE_AI_API_KEY`, `GITGOV_COPILOT_PROVIDER=google`, and `GITGOV_COPILOT_GOOGLE_MODEL=gemini-2.5-flash` configured without printing secret values; preview remains fallback-only unless explicitly configured later. Local validation passed with `pnpm run typecheck`, `pnpm run lint`, `pnpm run build`, and a local route smoke returning HTTP `200`, `success=true`, `mode=ai`, `model=google/gemini-2.5-flash`, `4` citations, `4` sources, `4` ok sources, and `0` warnings. Report: `docs/reports/google-ai-sdk-copilot-provider-2026-05-01.md`. After merge/deploy, run KAN-40 validator with `-RequireAiMode` against production.
