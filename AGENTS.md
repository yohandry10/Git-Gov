...
- Risk Tier Baseline Calibration scheduled run `24999681550` failed on 2026-04-27 because the workflow used PowerShell array splatting and passed `-RepoFullName` positionally into the `Tier` parameter. Desktop Updater Readiness had the same invocation pattern and produced optional job failures. Both workflows were corrected to hashtable splatting and locally validated with the target scripts. Follow-up manual run `25049577630` proved calibration succeeded but artifact upload needed an explicit UTF-8 write to `$GITHUB_OUTPUT` for `report_path`.
