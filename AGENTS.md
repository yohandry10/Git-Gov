...
- Reviewed Workflow Installation ticket `KAN-35` continues Enterprise Self-Service Adoption by adding a dry-run-first local installer for workflow template packs. Branch `product/KAN-35-reviewed-workflow-installation` adds `scripts/control-plane/install_enterprise_workflow_templates.ps1`, `docs/design/reviewed-workflow-installation-mvp.md`, and `docs/reports/reviewed-workflow-installation-2026-04-30.md`. The installer accepts CLI `-PackDir` output or dashboard `-PackPath` JSON, writes only under `.github/workflows`, requires explicit `-Apply`, requires `-Overwrite` for replacements, rejects unsafe paths, and does not read `.env`, provider tokens, or secret values. It does not mutate remote GitHub repositories. Direct provider checks, formal enterprise release approval, and Vercel AI SDK Copilot remain pending.
