...
- Governance Copilot AI mode validation ticket `KAN-40` starts the operational activation step after KAN-38/KAN-39. Branch `product/KAN-40-governance-copilot-ai-validation` adds `scripts/control-plane/validate_governance_copilot_ai_mode.ps1`, `.github/workflows/governance-copilot-ai-mode-validation.yml`, `docs/runbooks/governance-copilot-ai-mode-validation.md`, and `docs/reports/governance-copilot-ai-mode-validation-2026-04-30.md`. The validator is secret-safe: it calls the copilot route with `GITGOV_API_KEY`, records mode/source/citation counts, stores only answer length and SHA-256 hash, and supports `-RequireAiMode` for post-AI-Gateway/OIDC enforcement. Local production validation passed in non-strict `fallback` mode with HTTP `200`, `4` citations, `4` sources, and `4` ok sources; strict mode currently fails as expected because production AI Gateway/OIDC is not active.
