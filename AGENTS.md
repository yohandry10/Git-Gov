...
- Adoption Profile Persistence ticket `KAN-31` continues Enterprise Self-Service Adoption by persisting the KAN-30 profile per organization. It adds admin routes `GET/PUT /enterprise/adoption-profile`, backend JSON validation, Supabase migration `supabase_schema_v23.sql`, Tauri commands, dashboard save/load controls, `docs/design/adoption-profile-persistence-mvp.md`, and `docs/reports/adoption-profile-persistence-2026-04-30.md`. The saved profile remains secret-safe: it stores provider/module intent, policy preset, repository/default-branch/Jira key, and no token or `.env` values. Apply DB migration `v23` before using the endpoint in production.
